...
- Current Git history only contains “Initial commit,” so no established commit message convention exists.
...
- Store secrets in `.env.local` (not committed). Required: `DATABASE_URL` for Neon Postgres.
